...
// variables host check

variable "host_status_check_enabled" {
  description = "Flag to enable host status check monitor"
  type        = string
  default     = "true"
}

variable "host_status_check_extra_tags" {
  description = "Extra tags for host status check monitor"
  type        = list(string)
  default     = []
}
variable "unanswered_time_in_min" {
  default = "2"
}

variable "new_host_delay_in_sec" {
  default = "60"
}